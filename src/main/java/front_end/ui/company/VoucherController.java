...
public class VoucherController implements Initializable {
    ...
    private final VoucherBO voucherBo = (VoucherBO) BOFactory.getInstance().getBO(BOFactory.BOTypes.VOUCHER);
    ...
    private void submit() {
        if (RegexManager.verify_number(txt_number.getText())) {
            ...
            Theme.giveAWarning(
                    Session.isSinhala()
                            ? "වවුචර අංකය වලංගු නැත, කරුණාකර නැවත උත්සාහ කරන්න !"
                            : "Invalid Voucher Number, Please try again !",
                    CompanyDashboardController.windowMsg,
                    Session.admin_mainLabel,
                    Session.admin_regionBack,
                    Session.admin_regionTop,
    ...
    private void setFocusListeners() {
        ...
        txt_number.focusedProperty().addListener((__, ___, isFocused) ->
        {
            if (!isFocused && !txt_number.getText().equals("")) {
                if (RegexManager.verify_number(txt_number.getText())) {
                    try {
                        VoucherDTO voucher = voucherBo.searchVoucher(Integer.parseInt(txt_number.getText()));
                        if (null != voucher) {
                            datePicker_date.setValue(voucher.getDate());
                            txt_description.setText(voucher.getDescription());
                            txt_amount.setText(Integer.toString(voucher.getAmount()));
                            cmb_ledger.getSelectionModel().select(ledgerBO.search_byId(voucher.getLedger()).getName());
                            lbl_main.setText(Session.isSinhala()
                                    ? "වවුචර යාවත්කාලීන කිරීම"
                                    : "Update Voucher"
                            );
